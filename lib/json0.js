...
json.transformPresence = function(presence, op, isOwnOp) {
  ...
  for(let c of op) {
    ...
    // Transform against subtype ops only (for now).
    if (c.t) {
      const presencePath = presence.slice(0, presence.length - 2);
      if(json.canOpAffectPath(c, presencePath)) {
        const subPresenceType = presence[presence.length - 2];
        const subPresence = presence[presence.length - 1];
        // TODO test this check
        //if(subPresenceType === c.t) {
        const { transformPresence } = subtypes[subPresenceType];
        return presencePath.concat([
          subPresenceType,
          transformPresence(subPresence, c.o, isOwnOp)
        ]);
        //}
      }
    }
