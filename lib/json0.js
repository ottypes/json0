...
json.transformPresence = function(presence, op, isOwn) {
  console.group('json transform', presence, op);
  if (op.length < 1) {
    return presence;
  }
  const exOp = op[0];
  const opT = op[0].t;
  console.log('exop', exOp, opT, exOp.p);
  if (opT && subtypes[opT] && exOp.p) {
    if (!presence.p || !presence.p[0] || presence.p[0] !== exOp.p[0]) {
      console.log('creating presence', presence, exOp.p[0]);
      presence = {
        ...subtypes[opT].createPresence(),
        p: op[0].p,
        u: presence.u,
        t: op[0].t
      };
    ...
    presence = {
      ...subtypes[opT].transformPresence(presence, op, isOwn),
      p: op[0].p,
      t: op[0].t
    };
